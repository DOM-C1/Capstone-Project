...
def get_n_letter_words_w_freq(length_of_word):
    themes = ["nature", "city", "emotion", "colour", "food", "music", "travel", "technology", "art", "sports"]
...
async def arcade_game_loop(list_of_words, num_of_guesses):
  ...
  global flag
  list_of_guesses = []
  print(f"You have {num_of_guesses} guesses and {int(duration)} seconds to guess a word of length {word_length}")
  ...
  for guesses in range(num_of_guesses):
      if duration <= 0:
         print(f"Oops, you ran out of time, the correct word was {random_word}")
      ...
      while len(guess) != len(random_word) or is_valid_word(guess) == False:
        new_screen()
        print_list_of_guesses(list_of_guesses)
        print(f"Invalid input, please enter a valid {word_length} letter word")
        guess = await session.prompt_async("Player guess a word: ")
        guess = guess.lower()
...
while choice != "quit":
  ...
  if choice == "1":
    ...
    while duration > 0 and flag == True:
      avg_num_guesses = (len(list_of_guesses) + avg_num_guesses) / attempts if attempts != 0 else avg_num_guesses
  ...
  if choice == "2":
    ...
    while True:
      ...
      print()
      difficulty_select = input("Enter your choice (1, 2, 3, or 'quit'): ")
